...
def post_exam_results(request: HttpRequest):
    ...
    for i, question in enumerate(questions):
        correct_answer = Question.objects.filter(id=question["id"]).values(
...
def display_exam_results(request: HttpRequest):
    if request.session.has_key("score"):
        results = dumps(
            {"msg": "Success", "questions": request.session['questions'], "score": request.session['score'], "date": request.session['date']})
    else:
...
def application_form_view(request, code):
    unanswered_questions = Question.objects.filter(
        code_id=Code.objects.get(code_name=code), correct_answer__isnull=True).all()
    random_q = unanswered_questions.exclude(question__in=QuestionApplication.objects.all(
    ).values('question_id__question')).order_by('?').first()
